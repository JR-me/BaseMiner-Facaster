...
"""
deploy_baseminer.py
Compiles and deploys bBall → BaseMiner → MinerBadge,
then wires them together on Base mainnet.

Note: BaseMinerOnboard is not included in this deployment.
...
Security notes:
  ...
  I-03: --dry-run flag added. In dry-run mode the script estimates gas for every
        transaction, checks the deployer's ETH balance, and prints the full
...
def compile_contracts():
    ...
    sol_files  = [
        str(script_dir / "bBall.sol"),
        str(script_dir / "BaseMiner.sol"),
    ...
    cball_key = next(k for k in result if k.endswith(":bBall"))
    miner_key = next(k for k in result if k.endswith(":BaseMiner"))
...
def save_artifacts(addresses, abis):
    ...
    print(f"\n📄  ABIs saved: cball_abi.json, baseminer_abi.json, minerbadge_abi.json")
...
def dry_run(w3, sender, cball_art, miner_art, badge_art):
    ...
    steps = [
        ("bBall deploy",      lambda: estimate_deploy_gas(w3, cball_art["abi"], cball_art["bin"], [],              sender)),
        ...
        ("BaseMiner deploy",  lambda: estimate_deploy_gas(w3, miner_art["abi"], miner_art["bin"], [sender, sender], sender)),
        ("MinerBadge deploy", lambda: estimate_deploy_gas(w3, badge_art["abi"], badge_art["bin"], [sender, sender], sender)),
    ]

    total_gas = 0
    print(f"  {'Step':<30}  {'Gas (est.)':>12}  {'Cost (ETH)':>14}")
    print(f"  {'-'*30}  {'-'*12}  {'-'*14}")
    for label, estimator in steps:
        ...
        total_gas += gas
        print(f"  {label:<30}  {gas:>12,}  {cost_eth:>14.6f} ETH")
    ...
    print(f"  {'setMinter call':<30}  {set_minter_gas:>12,}  {set_minter_cost:>14.6f} ETH")
    ...
    print(f"  Total estimated cost: {total_cost_eth:.6f} ETH")
    print(f"  Deployer balance    : {balance_eth:.6f} ETH")

    if balance >= total_cost_wei:
        print(f"\n  ✅  Balance sufficient — {balance_eth - total_cost_eth:.6f} ETH will remain after deployment.")
    else:
        shortfall = (total_cost_wei - balance) / 1e18
        print(f"\n  ❌  Insufficient balance — need {shortfall:.6f} more ETH to deploy.")

    print("\n  Re-run without --dry-run to deploy.\n")


def main():
    parser = argparse.ArgumentParser(description="Deploy BaseMiner game contracts to Base mainnet")
    ...
    if chain_id != BASE_CHAIN_ID:
        print(f"⚠️   WARNING: Expected Base mainnet (chainId {BASE_CHAIN_ID}), got chainId {chain_id}")
    ...
    cball_address = deploy(w3, cball_art["abi"], cball_art["bin"], [],                           sender, private_key, "bBall")
    miner_address = deploy(w3, miner_art["abi"], miner_art["bin"], [cball_address, sender],      sender, private_key, "BaseMiner")
    badge_address = deploy(w3, badge_art["abi"], badge_art["bin"], [miner_address, sender],      sender, private_key, "MinerBadge")

    # Wire: set BaseMiner as bBall minter
    ...
    send_tx(w3, cball_contract.functions.setMinter(miner_address), sender, private_key,
            "Setting BaseMiner as bBall minter")
    ...
    addresses = {
        "bBall":     cball_address,
        "BaseMiner": miner_address,
        "MinerBadge": badge_address,
    }

    save_artifacts(addresses, {
        "bball":     cball_art["abi"],
        "baseminer": miner_art["abi"],
        "minerbadge": badge_art["abi"],
    })

    basescan_base = "https://basescan.org/address"

    print(f"""
╔══════════════════════════════════════════════════════════════════╗
║                BASEMINER DEPLOYMENT COMPLETE                      ║
╠══════════════════════════════════════════════════════════════════╣
║  Network:     Base Mainnet (chainId 8453)                         ║
║  bBall Token: {cball_address}
║  BaseMiner:   {miner_address}
║  MinerBadge:  {badge_address}
╠══════════════════════════════════════════════════════════════════╣
║  Next steps:                                                     ║
║  1. Paste addresses into index.html (top of <script>)            ║
║  2. Open index.html in a browser with MetaMask on Base           ║
║  3. Start mining!                                                ║
╚══════════════════════════════════════════════════════════════════╝
    """)

    print(f"  bBall:     {basescan_base}/{cball_address}")
    print(f"  BaseMiner: {basescan_base}/{miner_address}")
    print(f"  MinerBadge:{basescan_base}/{badge_address}")
